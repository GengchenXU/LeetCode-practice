...
那个状态就足够了，这样可以把空间复杂度降到 O(1)O(1)O(1):
```Java
// k == 1
...
```
两种方式都是一样的，不过这种编程方法简洁很多。但是如果没有前面状态转移方程的引导，是肯定看不懂的。后续的题目，我主要写这种空间复杂度 O(1)O(1)O(1) 的
解法。
第二题，k = +infinity
...
```
第五题，k = 2
k = 2 和前面题目的情况稍微不同，因为上面的情况都和 kkk 的关系不太大。要么 kkk 是正无穷，状态转移和 kkk 没关系了；要么 k = 1，跟 k = 0 这个 
basecasebase casebasecase 挨得近，最后也没有存在感。
